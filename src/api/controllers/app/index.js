...
const register = async (req, res) => {
    try {
        ...
        const validation = validatorMethod({ email, name, role_id, institute_id, password }, res);
        if (validation) {
            const find = await Users.findOne({ email });
            if (find) {
                res.status(200).json({
...
const createTeacherPage = async (req, res) => {
    try {
        const { user_id, department_id, section, subject_code, subject_description, semester, year, no_of_enrolled } = req.body;
        const validation = validatorMethod(
            { user_id, department_id, section, subject_code, subject_description, semester, year, no_of_enrolled },
            res
        );
        if (validation) {
